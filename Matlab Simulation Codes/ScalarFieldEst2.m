...
c= [1 1; 4 2; 1.6 3.1; -4 4; -3 -1; -1 -4; 3 -3; 4 -3.5;];  %centres of rbfs
bot_locx = [1 3 1.5 0];
...
while(loop)
   ...
   % disp((x-xt)^2 + (y-yt)^2);
    if((((x(1)-xt(1))^2 + (y(1)-yt(1))^2)<0.000005) && (tvi(1)<length(c1(:,1))))
        tvi(1) = tvi(1)+1;
    end
    if((((x(2)-xt(2))^2 + (y(2)-yt(2))^2)<0.000005) && (tvi(2)<length(c2(:,1))))
        tvi(2) = tvi(2)+1;
    end
    if((((x(3)-xt(3))^2 + (y(3)-yt(3))^2)<0.000005) && (tvi(3)<length(c3(:,1))))
        tvi(3) = tvi(3)+1;
    end
    if((((x(4)-xt(4))^2 + (y(4)-yt(4))^2)<0.000005) && (tvi(4)<length(c4(:,1))))
        tvi(4) = tvi(4)+1;
    end
       
    if(t >= 8 && (tvi(1) == (length(c1(:,1)))) && (tvi(2) == (length(c2(:,1)))) && (tvi(3) == (length(c3(:,1)))) && (tvi(4) == (length(c4(:,1)))))
        disp(a_cap);
        disp(t);
        loop = 0;
        %plot(X1,Y1)
